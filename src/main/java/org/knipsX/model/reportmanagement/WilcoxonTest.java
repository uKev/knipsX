...
public class WilcoxonTest {
	...
	public void setPictureContainer(
			final List<PictureContainer> pictureContainer) {
	...
	/**
	 * Shows if the Wilcoxontest is passed by the selected pictures and the
	 * significance. efore calling getResult() it has to be checked if the test
	 * is valid.
	 * 
	...
	/**
	 * Returns the result for the p value. Before calling getResult() it has to
	 * be checked if the test is valid.
	 * 
	 * @return p value.
	 */
	...
	public void calculate() {
		if ((!this.isActive) || (this.wilcoxenTestType == null)
				|| (this.pictureContainer == null) || (this.parameter == null)
				|| (this.pictureContainer.size() != 2)) {
			this.isValid = false;
		} else {

			this.initTest();

			if ((testData1.length == 0) || (testData2.length == 0)
					|| (testData1.length < 2) || (testData2.length < 2)
					|| (testData1.length > 5000) || (testData2.length > 5000)) {
				this.isValid = false;
			} else {
				RankSumTest wilcoxonTest1 = new RankSumTest();
				this.result = wilcoxonTest1.pValue(testType, testData1,
						testData2);
	...
	private void initTest() {
		...
		for (final PictureInterface picture : this.pictureContainer.get(0)) {
			try {
				testData1Vector.add(Converter.objectToDouble(picture
						.getExifParameter(this.parameter)));
			} catch (ClassCastException e) {
				testData1Vector.add(0.00d);
			}
		}

		for (final PictureInterface picture : this.pictureContainer.get(1)) {
			try {
				testData2Vector.add(Converter.objectToDouble(picture
						.getExifParameter(this.parameter)));
			} catch (ClassCastException e) {
				testData2Vector.add(0.00d);
			}
		}
