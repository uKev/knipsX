...
\label{subsec:programmausfuehrung}
	...
	\begin{description}
		...
		\item[/F065/] \textit{Lexikographisches Sortieren:}\par	Beim \gls{lexgraph} Sortieren gilt allgemein im gesamten Programm, dass nach den java.text.Collator Rules von SUN, welche abhängig sind von Locale, sortiert wird.
		
		\item[/F070/] \textit{Namensvergabe Allgemein:}\par	Beim Ausfüllen von Textfeldern gilt allgemein im gesamten Programm es darf der ASCII-Zeichensatz verwendet werden bis auf folgende Zeichen: "`/"', "`\textbackslash"', "`:"', "`*"', "`?"', "`"'"', "`>"', "`<"', "`|"'.
