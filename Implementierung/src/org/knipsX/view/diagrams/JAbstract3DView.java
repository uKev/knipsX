...
public abstract class JAbstract3DView<M extends AbstractModel> extends JAbstractDiagram<M> {
     ...
     */
    protected final static int NUMBEROFSEGMENTS = 10;
	...
	for (int q = 0; q <= 10; q++) {
	    for (int i = 0; i <= 2; i++) {
		if (i == 0)
		    createText(new Vector3d(-1, q, -0.75d), new Vector3d(size, size, size), basicMaterial(1, 1, 1),
			    String.valueOf(q + 1) + ".11.09");
		if (i == 1)
		    if (q > 0)
			createText(new Vector3d(-1, -0.5d, q), new Vector3d(size, size, size), basicMaterial(1, 1, 1),
				String.valueOf(q * 100));
		if (i == 2)
		    if (q > 0)
			createText(new Vector3d(q, 0, -1), new Vector3d(size, size, size), basicMaterial(1, 1, 1),
				String.valueOf((q + 17)));
	    }
	}
	...
	for (int q = 0; q <= 0; q++) {
		...
		for (int i = 0; i <= this.GRIDDENSITYFACTOR * (int) this.AXISSIZE; i++) {
			...
			gridTrans.setTranslation(new Vector3d((double) i / (this.GRIDDENSITYFACTOR), 0,
				this.AXISSIZE / 2.0));
		    } else if (p == 1) {
			gridTrans.rotZ(90 * Math.PI / 180.0d);
			gridTrans.setTranslation(new Vector3d(this.AXISSIZE / 2.0, 0, (double) i
