...
public class JBoxplot<M extends AbstractReportModel> extends JAbstract2DDiagram<M> {
    ...
    public void generateContent() {
        ...
        Boxplot[] myboxplot = new Boxplot[3];
        ...
        for (int p = 0; p < myboxplot.length; p++) {
            if (p % 2 == 0) {
                ...
                myboxplot[p] = new Boxplot(0.05490246, 0.0775161, 0.7477903, -0.5993204, 2.797647, -2.462955, null,
        ...
        this.axis3D[1].setAxisSize(Math.max(2 * myboxplot.length, 10));
        ...
        for (int i = 0; i < myboxplot.length; i++) {
            ...
            double interquartileRange = Math.abs(myboxplot[i].getUpperQuartile())
                    + Math.abs(myboxplot[i].getLowerQuartile());
            double middleOfinterQuartileRange = (myboxplot[i].getLowerQuartile() + myboxplot[i].getUpperQuartile()) / 2;

            /* Create inner quartile range */
            this.createCube(new Vector3d(0, getAxisSpace(middleOfinterQuartileRange), xSpace), new Vector3d(boxWidth,
                    (interquartileRange / 2) * scaleFactor, boxWidth), this.basicMaterial(boxplotColors[i
                    % boxplotColors.length]));

            double middleOfUpperWhiskerAndUpperQuartile = (myboxplot[i].getUpperQuartile() + myboxplot[i]
                    .getUpperWhisker()) / 2;
            double upperWhiskerRange = Math.abs(myboxplot[i].getUpperWhisker())
                    - Math.abs(myboxplot[i].getUpperQuartile());

            /* Create upper whisker */
            this.createCube(new Vector3d(0, getAxisSpace(middleOfUpperWhiskerAndUpperQuartile), xSpace), new Vector3d(
                    whiskerScale, (upperWhiskerRange / 2) * scaleFactor, whiskerScale), this.basicMaterial(0, 0, 0));
            this.createCube(new Vector3d(0, getAxisSpace(myboxplot[i].getUpperWhisker()), xSpace), new Vector3d(
                    whiskerScaleWidth, whiskerScale, whiskerScale), this.basicMaterial(0, 0, 0));

            double middleOfLowerWhiskerAndLowerQuartile = (myboxplot[i].getLowerQuartile() + myboxplot[i]
                    .getLowerWhisker()) / 2;
            double lowerWhiskerRange = Math.abs(myboxplot[i].getLowerWhisker())
                    - Math.abs(myboxplot[i].getLowerQuartile());

            /* Create lower whisker */
            this.createCube(new Vector3d(0, getAxisSpace(middleOfLowerWhiskerAndLowerQuartile), xSpace), new Vector3d(
                    whiskerScale, (lowerWhiskerRange / 2) * scaleFactor, whiskerScale), this.basicMaterial(0, 0, 0));
            this.createCube(new Vector3d(0, getAxisSpace(myboxplot[i].getLowerWhisker()), xSpace), new Vector3d(
                    whiskerScaleWidth, whiskerScale, whiskerScale), this.basicMaterial(0, 0, 0));

            /* Create mean */
            this.createSphere(new Vector3d(-boxWidth, getAxisSpace(myboxplot[i].getMean()), xSpace), new Vector3d(
                    boxWidth / 4, boxWidth / 4, whiskerScale), this.basicMaterial(0, 0, 0));

            /* Create median */
            this.createCube(new Vector3d(-boxWidth, getAxisSpace(myboxplot[i].getMedian()), xSpace), new Vector3d(
                    boxWidth, whiskerScale, whiskerScale), this.basicMaterial(1, 0, 0));

            /* Create outliers */
            if (myboxplot[i].getOutlier() != null) {
                for (int p = 0; p < myboxplot[i].getOutlier().size(); p++) {
                    this.createSphere(new Vector3d(0, getAxisSpace(myboxplot[i].getOutlier().get(p)), xSpace),
                            new Vector3d(boxWidth / 5, boxWidth / 5, whiskerScale), this.basicMaterial(1, 1, 1));
                }
            }

            /* Create tick on the x axis */
            this.createCube(new Vector3d(0, -0.125, xSpace), new Vector3d(0.025, 0.125, 0.025), this.basicMaterial(1, 1,
                    1));
            ...
            this.createText(new Vector3d(0, -1, xSpace), new Vector3d(textSize, textSize, textSize), this
                    .basicMaterial(1, 1, 1), myboxplot[i].getPictureSetName());
        }
