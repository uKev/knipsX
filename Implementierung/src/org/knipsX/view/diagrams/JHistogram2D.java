...
import org.knipsX.model.reportmanagement.Bar;
        ...
        if (model != null && this.model.isModelValid()) {
            ...
            for (int i = 0; i < categories.length; i++) {
                ...
                logger.debug("Category Number " + category + " Min X "
                        ...
                        + this.getxAxis().getAxisSpace(categories[i].getMaxValueX()) + " Heigth of Bar(s): ");
                for (Bar bar : categories[i].getBars()) {
                    logger.debug(bar.getHeight());
                }
                ...
                if (categories[i].getBars().get(0).getHeight() > 0) {
                    ...
                    this.createCube(new Vector3d(0, 0, xPosition),
                            new Vector3d(xRange / 2 * shrinkFactor, barHeight, 1), this.basicMaterial(Color.orange));

                    /* Create the actual number of elements on top of the bar */
                    double size = 0.33d;
                    this.createText(new Vector3d(0, barHeight + 0.125, xPosition), new Vector3d(size, size, size), this
                            .basicMaterial(Color.white), Integer.toString((int) categories[i].getBars().get(0)
                            .getHeight()));
                }
                category++;
