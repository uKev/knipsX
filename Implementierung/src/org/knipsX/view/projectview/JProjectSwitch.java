...
/**
 * Represents the view for a dialog which gives the user the possibility to add
 * a new report.
 */
public class JProjectSwitch extends JAbstractDialog {
	...
	private void configure() {
		...
		this.jButtonConfirm
				.addActionListener(new SwitchProjectConfirmController(
						this.model));
		...
		/* create an action listener (which knows the model) to the button */
		this.jButtonRefuse.addActionListener(new SwitchProjectRefuseController(
				this.model));
		...
		/* create an action listener (which knows the model) to the button */
		this.jButtonCancel.addActionListener(new SwitchProjectCancelController(
				this.model));
