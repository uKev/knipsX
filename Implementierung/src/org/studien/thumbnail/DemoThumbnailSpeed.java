...
public class DemoThumbnailSpeed {
	...
	public static void main(String[] args) throws IOException {
		...
		for (int i = 0; i < 10; i++) {
			rawImage = ImageIO.read(new File(tPath + "DSC00970_"
					+ i + ".JPG"));
			measurepoint("Eingelesen");
			// scale and save the thumbnail
			// ImageIO.write(DemoThumbnailQuality.getThumbOf(rawImage, 200,
			// Image.SCALE_FAST), "jpg", new File(tPath +
			// "10MegTestPic_thumb_default.jpg"));
		...
		/*
		 * ImageIO.write(temp, "jpg", new File(tPath +
		 * "10MegTestPic_thumb_fast.jpg"));
		 */
	...
	public static Image getThumbOf(BufferedImage bImage, int maxWidthOrHight,
			int hints) {
		...
		if (width >= height) {
			...
			height = -1; // negative Höhe bedeutet, dass diese Höhe dem
							// Seitenverhältnis entsprechend an die neue Breite
							// angepasst wird.
		} else {
			width = -1; // negative breite: Breite wird dem Seitenverhältnis
						// entsprechend an die neue Höhe angepasst.
		...
		/*
		 * BufferedImage bThumb = new
		 * BufferedImage(thumbnailed.getWidth(null),thumbnailed.getHeight(null),
		 * bImage.getType()); measurepoint("Buffered Image erzeugt");
		 * 
		 * 
	...
	static void measurepoint(String comment) {
		...
		System.out.println("Zeit benötigt (letzter Schritt): " + ((zstDiff))
				+ " milisec");
		System.out.println("Zeit benötigt (letzter Schritt): "
				+ ((zstDiff) / 1000) + " sec");
		System.out.println("Zeit benötigt (gesamt): " + ((zstGesamt))
				+ " milisec");
		System.out.println("Zeit benötigt (gesamt): " + ((zstGesamt) / 1000)
				+ " sec");
		System.out.println();
